 ...
 */
 ...
 /**
 * @file Updated by TU Darmstadt Students for PTA Project
 * @author  Abbas & Juzer
 *
 */
...
(function (sandbox) {
    ...
    function MyAnalysis() {
       var noTimesEvalCalled = 0;	   
	   var noTimesAddCalled = 0;
	   var noTimesDelCalled = 0;
       var noTimesPrototypeCalled = 0;
       var noTimesOverridingCalled = 0;
       var protoTypeValueId = 0;
       var globalMapObject = new Map();  
       var ObjectNamesForOverriding = new Map();  
       var PrototypesMap = [];  
       var PrototypesMapClone = []; 
       var branches = {};
       var AllObjects = new Object();
       var relationalMapping = [];
	   var ObjectIID = 0;
	   var objId = 0; 
	   var objAdd = null;
	   ...
	   function ObjValue(v) {
        ...
        this.declare = function (iid, name, val, isArgument, argumentIndex, isCatchParam) {
			...
			if (typeof(val) == "function"){
                var idOfClass = sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(val));
                ObjectNamesForOverriding.set(idOfClass,name);
        ...
        this.getField = function (iid, base, offset, val, isComputed, isOpAssign, isMethodCall) {
          if (offset == "prototype" || iid != null)
			{
				protoTypeValueId  = sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(val));
        ...
        this.putFieldPre = function (iid, base, offset, val, isComputed, isOpAssign) {
			...
			objAddition = base.hasOwnProperty(offset);
						
			if(typeof(base) == 'object'){
				if(typeof(val) == 'function' && base.hasOwnProperty(offset)){
					noTimesOverridingCalled++;
				}
			}
        ...
        this.putField = function (iid, base, offset, val, isComputed, isOpAssign) {
			var baseId= sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(base));
          		   
		   	if(typeof baseId !== "undefined")
			{
				var objName = AllObjects[baseId];
				if(typeof objName !== "undefined")
				{	
					if(	ObjectIID == iid && objId == baseId && objAddition != true){
						noTimesAddCalled++;
					}
        ...
        this.read = function (iid, name, val, isGlobal, isScriptLocal) {
            ...
            if(globalMapObject.has(name) && val != null && val !== undefined && typeof(val) == "object" ){
          
                var globalHashkeyz = Object.keys(globalMapObject.get(name)).toString();
                var updatedObjectValue= Object.keys(val).toString();
                if (globalHashkeyz != updatedObjectValue){
                    if (globalHashkeyz.split(',').length > updatedObjectValue.split(',').length ){
                            noTimesDelCalled++;
                    }
					else if (globalHashkeyz.split(',').length < updatedObjectValue.split(',').length ){
                        noTimesAddCalled++;
                    
					}
                globalMapObject.delete(name);
                globalMapObject.set(name, Object.assign({}, val));
        ...
        this.endExecution = function () {
              ...
              console.log("-----------------------------------------------------");
			  console.log("No. of times eval() called : "+noTimesEvalCalled) ;
			  console.log("-----------------------------------------------------");
			  console.log("**************************************************************");	
			  console.log("********************Delete count******************************");
              console.log("-----------------------------------------------------");
			  console.log("No. of times dynamic delete called : "+noTimesDelCalled) ;
			  console.log("-----------------------------------------------------");
			  console.log("**************************************************************");
			  console.log("********************Add count****************************&****");
              console.log("-----------------------------------------------------");
			  console.log("No. of times dynamic add called : "+noTimesAddCalled) ;
			  console.log("-----------------------------------------------------");
			  console.log("**************************************************************");
			  console.log("********************Prototype Update count********************");
              console.log("-----------------------------------------------------");
			  console.log("No. of times Prototype Update called : "+noTimesPrototypeCalled) ;
			  console.log("-----------------------------------------------------");
			  console.log("**************************************************************");
			  console.log("********************Method OverRiding count*******************");
              console.log("-----------------------------------------------------");
			  console.log("No. of times Method over-riding called : "+noTimesOverridingCalled) ;
			  console.log("-----------------------------------------------------");
			  console.log("**************************************************************");
